  ...
  router.post("/", (req, res) => {
    ...
    db.query(orderIdQuery, values)
      .then(orderResponse => {
        ...
        for (
          let orderIndex = 0;
          orderIndex < restaurantAndItemIds.itemId.length;
          orderIndex++
        ) {
          if (restaurantAndItemIds.quantity[orderIndex] !== "0") {
            ...
            let orderItemValues = [
              orderId,
              restaurantAndItemIds.itemId[orderIndex],
              restaurantAndItemIds.quantity[orderIndex]
            ];
        ...
        Promise.all(queries)
          .then(insertedOrderData => {
            ...
            db.query(queryString, queryParams).then(resultSet => {
